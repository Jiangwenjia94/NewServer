...
/* ----------- local functions - only ever used internally by Orbit ---------- */
static void dpper
(
...
static void dsinit
(
//Orbitfix no longer needed pass in xke
...
namespace OrbitFuncs
{
		...
		)
	{
		...
		if (init == 'n')
		{
			...
			else
			{
				...
				//  Orbitfix for afspc written intrinsic functions
	...
	*    vallado, crawford, hujsak, kelso  2006
	...
	static void dsinit
		(
		// Orbitfix just send in xke as a constant and eliminate getgravconst call
		// gravconsttype whichconst, 
		double xke,
		double cosim, double emsq, double argpo, double s1, double s2,
		double s3, double s4, double s5, double sinim, double ss1,
		...
		)
	{
		...
		// Orbitfix identify constants and allow alternate values
		...
		nodem = nodem + dnodt * t;
		mm = mm + dmdt * t;
		//   Orbitfix for negative inclinations
		...
		if (irez != 0)
		{
			...
			/* ------------ for Orbit, initialize the integrator ---------- */
			xli = xlamo;
			xni = no;
			atime = 0.0;
			nm = no + dndt;
	...
	*    vallado, crawford, hujsak, kelso  2006
		...
		)
	{
		...
		/* ------------------------- epoch restart ----------------------  */
		//   Orbitfix for propagator problems
		//   the following integration works for negative time steps and periods
		//   the specific changes are unknown because the original code was so convoluted

		// Orbitfix take out atime = 0.0 and fix for faster operation
		ft = 0.0;
		if (irez != 0)
		{
			// Orbitfix streamline check
			...
			// Orbitfix move check outside loop
			if (t > 0.0)
				delt = stepp;
			else
				delt = stepn;
			...
			while (iretn == 381)
			{
				...
				/* ----------------------- integrator ------------------- */
				// Orbitfix move end checks to end of routine
	...
	*    vallado, crawford, hujsak, kelso  2006
	...
	static void initl
		(
		// Orbitfix satn not needed. include in satrec in case needed later  
		// int satn,      
		// Orbitfix just pass in xke and j2
		// gravconsttype whichconst, 
		double xke, double j2,
		...
		)
	{
		...
		// Orbitfix use old way of finding gst
		double ds70;
		...
		// Orbitfix modern approach to finding sidereal time
		//   if (opsmode == 'a')
		//      {
		// Orbitfix use old way of finding gst
	...
	*                             procedure Orbitinit
	*
	*  this procedure initializes variables for Orbit.
	*
	*  author        : david vallado                  719-573-2600   28 jun 2005
	*
	*  inputs        :
	...
	*    bstar       - Orbit type drag coefficient              kg/m2er
	*    ecco        - eccentricity
	*    epoch       - epoch time in days from jan 0, 1950. 0 hr
	...
	*    vallado, crawford, hujsak, kelso  2006
		...
		)
	{
		...
		/* ------------------------ initialization --------------------- */
		// Orbitfix divisor for divide by zero check on inclination
		...
		// Orbitfix - note the following variables are also passed directly via satrec.
		// it is possible to streamline the Orbitinit call by deleting the "x"
		// variables, but the user would need to set the satrec.* values first. we
		// include the additional assignments in case twoline2rv is not used.
		satrec.bstar = xbstar;
		// Orbitfix allow additional parameters in the struct
		satrec.ndot = xndot;
		satrec.nddot = xnddot;
		satrec.ecco = xecco;
		satrec.argpo = xargpo;
		satrec.inclo = xinclo;
		satrec.mo = xmo;
		// Orbitfix rename variables to clarify which mean motion is intended
		...
		/* ------------------------ earth constants ----------------------- */
		// Orbitfix identify constants and allow alternate values no longer needed
		// getgravconst( whichconst, tumin, mu, radiusearthkm, xke, j2, j3, j4, j3oj2 );
		ss = 78.0 / satrec.radiusearthkm + 1.0;
		// Orbitfix use multiply for speed instead of pow
		...
		// Orbitfix remove satn as it is not needed in initl
		initl
		...
		// Orbitfix remove this check as it is unnecessary
		// the mrt check in Orbit handles decaying satellite cases even if the starting
		...
		if ((omeosq >= 0.0) || (satrec.no_unkozai >= 0.0))
		{
			...
			// Orbitfix for divide by zero with xinco = 180 deg
		...
		// Orbitfix take out check to let satellites process until they are actually below earth surface
		//       if(satrec.error == 0)
		Orbit(satrec, 0.0, r, v);

		satrec.init = 'n';

		//#include "debug6.cpp"
		//Orbitfix return boolean. satrec.error contains any error codes
		return true;
	}  // Orbitinit
	...
	*                             procedure Orbit
	*
	*  this procedure is the Orbit prediction model from space command. this is an
	*    updated and combined version of Orbit and sdp4, which were originally
	...
	*
	*  inputs        :
	*    satrec	 - initialised structure from Orbitinit() call.
	...
	*    vallado, crawford, hujsak, kelso  2006
		...
		)
	{
		...
		if (satrec.isimp != 1)
		{
			...
			// Orbitfix use mutliply for speed instead of pow
		...
		// Orbitfix am is fixed from the previous nm check
		if ((em >= 1.0) || (em < -0.001)/* || (am < 0.95)*/)
		{
			//         printf("# error em %f\n", em);
			satrec.error = 1;
			// Orbitfix to return if there is an error in eccentricity
			return false;
		}
		// Orbitfix fix tolerance to avoid a divide by zero
		if (em < 1.0e-6)
			em = 1.0e-6;
		...
		// Orbitfix recover singly averaged mean elements
		satrec.am = am;
		satrec.em = em;
		satrec.im = inclm;
		satrec.Om = nodem;
		...
		if (satrec.method == 'd')
		{
			...
			// Orbitfix for divide by zero for xincp = 180 deg
		...
		// Orbitfix for decaying satellites
		if (mrt < 1.0)
		{
	...
	*    variables and initializes the Orbit variables. several intermediate varaibles
	...
	*
	*  outputs       :
	*    satrec      - structure containing all the Orbit satellite information
	*
	*  coupling      :
	*    getgravconst-
	*    days2mdhms  - conversion of days to month, day, hour, minute, second
	*    jday        - convert day month year hour minute second into julian date
	*    Orbitinit    - initialize the Orbit variables
	*
	*  references    :
	*    norad spacetrack report #3
	*    vallado, crawford, hujsak, kelso  2006
		...
		)
	{
		...
		// Orbitfix not needed here
		// satrec.alta = satrec.a*(1.0 + satrec.ecco) - 1.0;
		// satrec.altp = satrec.a*(1.0 - satrec.ecco) - 1.0;

		// ----------------------------------------------------------------
		// find Orbitepoch time of element set
		// remember that Orbit uses units of days from 0 jan 1950 (Orbitepoch)
		...
		// ---------------- initialize the orbit at Orbitepoch -------------------
		Orbitinit(whichconst, opsmode, satrec.satnum, (satrec.jdsatepoch + satrec.jdsatepochF) - 2433281.5, satrec.bstar,
...
int TEME2ECEF(double jd_utc, double *rs, double *rse) {
	...
	gm = JD2GMST(jd_utc);
	gmst = gm*pi / 180;
	rse[0] = rs[0] * cos(gmst) + rs[1] * sin(gmst);
	rse[1] = rs[1] * cos(gmst) - rs[0] * sin(gmst);
	rse[2] = rs[2];
	return 0;
}
